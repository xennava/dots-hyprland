...
Singleton {
    ...
    property string failMessage: Translation.tr(
    ...
    property var providerList: Object.keys(providers).filter(provider => provider !== "system"
    ...
    property var providers: {
        ...
        "danbooru": {
            ...
            "tagSearchTemplate":
            "https://danbooru.donmai.us/tags.json?limit=10&search[name_matches]={{query}}*",
            "tagMapFunc": response => {
                return response.map(item => {
                    return {
                        "name": item.name,
        ...
        "gelbooru": {
            ...
            "description": Translation.tr(
                               "The hentai one | Great quantity, a lot of NSFW, quality varies wildly"),
            "mapFunc": response => {
            ...
            "tagSearchTemplate":
            "https://gelbooru.com/index.php?page=dapi&s=tag&q=index&json=1&orderby=count&limit=10&name_pattern={{query}}%",
            "tagMapFunc": response => {
    ...
    function getWorkingImageSource(url) {
        if (url?.includes('pximg.net')) {
            return `https://www.pixiv.net/en/artworks/${url.substring(url.lastIndexOf('/') + 1).replace(
                        /_p\d+\.(png|jpg|jpeg|gif)$/, '')}`;
        }
        return url;
    }
    ...
    function setProvider(provider) {
        ...
        if (providerList.indexOf(provider) !== -1) {
            ...
            root.addSystemMessage(Translation.tr("Provider set to ") + providers[provider].name + (provider
                                                                                                   == "zerochan"
                                                                                                   ? Translation.tr(
